...
export function removeCube(element: any): void {
  // debugger;
  const blockInfo = thisSceneContains.filter((e: any) => e.position.x == element.position.x && e.position.y == element.position.y && e.position.z == element.position.z)
  if (blockInfo[0]) {
  ...
  if (!shiftDown && Scene.mode !== 'Inspect') {
    ...
    if (placeInfo) {
      if (placeInfo.object.name !== "helpPlane" && placeInfo.object.name !== "hoverBlock") {
        ...
        if (element.parent && (element.parent.children.length == 3 || element.name == "slabs")) {
